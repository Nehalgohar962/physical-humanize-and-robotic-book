...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ sessionId, onSessionChange }) => {
  const [messages, setMessages] = useState<Message[]>([
    { id: "1", role: "assistant", content: "Hello! I'm your AI Assistant for Physical AI & Humanoid Robotics. How can I help you today?", timestamp: new Date() },
  ]);
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { id: Date.now().toString(), role: "assistant", content: `You asked: ${userInput}`, timestamp: new Date() }]);
    }, 800);
  };
