    ...
    const animateProgressBars = () => {
      ...
      setTimeout(() => {
        ...
        progressBars.forEach(bar => {
          ...
          // Reset to 0 first to ensure animation
          bar.style.width = '0';
          // Trigger reflow
          void bar.offsetWidth;
          // Set to target percentage with transition
      ...
      // Set fallback width after animation duration to ensure visibility
      setTimeout(() => {
        const progressBars = document.querySelectorAll('.progress-fill');
        progressBars.forEach(bar => {
          const percent = bar.getAttribute('data-percent');
          // Ensure the bar is visible even if animation didn't work
          if (bar.style.width === '0px' || !bar.style.width) {
            bar.style.width = percent + '%';
          }
        });
      }, 2200); // 2 seconds animation + 200ms buffer
    };
