#!/usr/bin/env python3
"""
Robust ingestion script for actual book content from MDX files
This script will load, chunk, embed, and store the actual book content from MDX files with proper error handling.
"""
import sys
import os
...
def ingest_book_content():
    """Ingest the actual book content into the RAG system with proper error handling"""
    print("Starting robust ingestion of actual book content...")
    print("=" * 60)
    ...
    for i, module_content in enumerate(all_modules_content, 1):
        ...
        # Process and store each chunk with error handling and rate limiting
        success_count = 0
        for j, chunk in enumerate(chunked_content):
            # Add delay to prevent rate limiting
            if j > 0:
                time.sleep(1)  # 1 second delay between requests to prevent rate limiting
            ...
            try:
                ...
                # Add extra delay if we encounter an error
                time.sleep(5)
...
if __name__ == "__main__":
    print("Robust Book Content Ingestion System")
    ...
    print("Rate limiting and error handling are implemented to handle API constraints.\n")
