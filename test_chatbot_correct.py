...
def test_chatbot_api():
    print("Testing RAG Chatbot API with correct endpoints...")
    print("="*60)

    # Step 1: Create a session using the correct endpoint (chat router)
    print("1. Creating a new session via chat router...")
    session_response = requests.post(f"{BASE_URL}/api/chat/sessions")
    if session_response.status_code == 200:
        session_data = session_response.json()
        session_id = session_data['session_id']  # Note: response format may be different
    ...
    if chat_response2.status_code == 200:
        ...
        print(f"   Error details: {chat_response2.text}")

    print("\n" + "="*60)
    print("Chatbot API test completed!")
    print(f"API is accessible at: {BASE_URL}")
    print(f"Documentation: {BASE_URL}/docs")
    print("\nTo interact with the chatbot, you can use the web interface at:")
